...
> [!NOTE]
> `--keepalive` sets how often pings are sent; `--timeout` sets how long the agent waits for a reply before restarting (see [agent/tunnels]({{< ref "02-agent/01-tunnels" >}})). On slow transports such as DNS, raise `--timeout` so normal round-trip latency is not mistaken for a dead connection.
